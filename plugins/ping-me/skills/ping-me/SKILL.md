...
### Turn Completion (Stop Hook)

When Claude finishes a turn that took longer than 30 seconds, a TTS notification plays automatically with a summary of what was accomplished. If `OPENAI_API_KEY` is set, the summary is generated by GPT-4o-mini from the session transcript. Otherwise, a generic "Task complete" message plays.

Configure the time threshold with `PING_ME_THRESHOLD` (seconds, default: 30).

### Permission Requests

When Claude needs permission to run a tool, a TTS notification plays with context about what permission is being requested (e.g., "Permission needed for Bash: git push origin main").
...
- `OPENAI_API_KEY` - Enables AI-generated summaries for stop notifications (uses GPT-4o-mini)
- `PING_ME_THRESHOLD` - Minimum turn duration in seconds before stop notification fires (default: 30)
