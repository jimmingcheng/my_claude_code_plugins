...
import { SimpleLogger, generateContextualMessage, cleanupTempFiles } from './utils';
import { SkillInput, SkillResult, PluginConfig } from './types';

// Initialize logger
const logger = new SimpleLogger('[Ping-Me]');

/**
 * Parse command line arguments for skill input
 */
function parseSkillInput(): SkillInput {
 ...
 */
async function executeSkill(): Promise<SkillResult> {
  ...
  try {
    ...
    const skillInput: SkillInput = parseSkillInput();

    logger.debug(`Received skill input:`, skillInput);

    // Generate contextual notification message
    const message = generateContextualMessage(skillInput.message, skillInput.reason);
